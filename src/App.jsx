...
function isHoliday(date) { const md = `${String(date.getMonth()+1).padStart(2,"0")}-${String(date.getDate()).padStart(2,"0")}`; return HOLIDAYS.includes(md); }
function isWeekend(date) { const d = date.getDay(); return d === 0 || d === 6 || isHoliday(date); }
function getScheduleType(date) { return isWeekend(date) ? "weekend" : "weekday"; }
...
// ── Web Audio ───────────────────────────────────────────────
// iOSはユーザー操作起点でAudioContextを unlock する必要がある
let _audioCtx = null;
function getAudioCtx() {
  if (!_audioCtx) {
    _audioCtx = new (window.AudioContext || window.webkitAudioContext)();
  }
  // suspendedなら resume（iOS対策）
...
function sendNotif(title, body) {
  if (notifPermission === "granted") { try { new Notification(title, { body }); } catch {} }
}

// ── BigRing ─────────────────────────────────────────────────
function BigRing({ progress, running, finished, children }) {
  const r = 130, cx = 150, cy = 150, circ = 2 * Math.PI * r;
  ...
  return (
    <div style={{ position: "relative", width: 300, height: 300 }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", animation: running && !finished ? "breathe 3s ease-in-out infinite" : "none" }}>
        {children}
      </div>
    </div>
  );
...
// ── FullscreenTimer ─────────────────────────────────────────
function FullscreenTimer({ routine, routines, activeIndex, elapsed, running, done, onPause, onResume, onComplete, onSkip, totalDone, totalRemainSec }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "linear-gradient(180deg,#060E1A 0%,#0D1B2A 50%,#060E1A 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", padding: "48px 24px 40px", overflowY: "auto" }}>
      {/* 進捗バー */}
      <div style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
        <div style={{ display: "flex", gap: 6 }}>
          {routines.map((r, i) => (
            <div key={r.id} style={{ height: 4, borderRadius: 2, flex: 1, maxWidth: 40, background: i < totalDone ? "#4ECDC4" : i === activeIndex ? (running ? "#FF6B6B" : "#8BB4D8") : "#1E3A5F", transition: "background 0.3s" }} />
          ))}
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 24, flex: 1, justifyContent: "center" }}>
        <BigRing progress={done ? 1 : progress} running={running} finished={finished || done}>
          ...
          <div style={{ fontSize: finished || done ? 36 : 52, fontWeight: 900, color: finished || done ? "#4ECDC4" : running ? "#FF6B6B" : "#F7F9FC", letterSpacing: -2, fontVariantNumeric: "tabular-nums", lineHeight: 1.1, marginTop: 4 }}>
...
function TabBar({ tab, setTab }) {
  return (
    <div style={{ display: "flex", background: "#0A1628", borderTop: "1px solid #1E3A5F", position: "fixed", bottom: 0, left: 0, right: 0, maxWidth: 430, margin: "0 auto", zIndex: 100 }}>
      {[{ key: "today", label: "今日", icon: "✅" }, { key: "routines", label: "設定", icon: "⚙️" }, { key: "log", label: "ログ", icon: "📊" }].map(t => (
        <button key={t.key} onClick={() => setTab(t.key)} style={{ flex: 1, padding: "12px 0 20px", background: "none", border: "none", color: tab === t.key ? "#4ECDC4" : "#4A6FA5", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, cursor: "pointer", fontSize: 11, fontWeight: tab === t.key ? 700 : 400 }}>
...
function TodayPage({ weekday, weekend, logs, setLogs, comments, setComments }) {
  ...
  const [scheduleType, setScheduleType] = useState(() => {
    return load(`rt_schedule_override_${today}`) || autoType;
  });
  ...
  function handleTypeChange(type) {
    ...
    save(`rt_schedule_override_${today}`, type);
  }
  ...
  const [missedMsg, setMissedMsg] = useState(null); // バックグラウンド中に終了した通知
  const timerRef = useRef(null);
  const startedAtRef = useRef(null);   // タイマー開始時刻（Date.now()）
  const baseElapsedRef = useRef(0);    // 一時停止時点までの経過秒
  const activeIndexRef = useRef(null); // visibilitychange用
  ...
  function startTimer(index) {
    unlockAudio(); // iOSのAudioContext unlock（ユーザー操作起点で必ず呼ぶ）
    baseElapsedRef.current = 0;
    startedAtRef.current = Date.now();
    activeIndexRef.current = index;
    setElapsed(0); setActiveIndex(index); setRunning(true); setShowFull(true);
    setMissedMsg(null);
    // localStorageに状態を保存（バックグラウンド復帰用）
  ...
  useEffect(() => {
    function onVisibilityChange() {
      ...
      if (realElapsed >= routine.duration) {
        ...
        startedAtRef.current = null;
        save("rt_timer_state", null);
        const overSec = realElapsed - routine.duration;
        setMissedMsg(`${routine.icon} ${routine.name} が ${overSec < 60 ? `${overSec}秒` : `${Math.floor(overSec/60)}分`}前に終了しました`);
        playFinishSound();
      } else {
        // まだ実行中 → タイマー再接続
        setRunning(true);
  ...
  const isWeekendDay = autoType === "weekend";
    ...
    <>
      {showFull && activeRoutine && (
        <FullscreenTimer routine={activeRoutine} routines={routines} activeIndex={activeIndex}
          ...
          onSkip={handleSkip} totalDone={doneCount} totalRemainSec={totalRemainSec} />
      )}
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {[{ key: "weekday", label: "📅 平日", }, { key: "weekend", label: "🌅 土日祝" }].map(({ key, label }) => (
            <button key={key} onClick={() => handleTypeChange(key)} style={{
              ...
              boxShadow: scheduleType === key ? "0 0 0 2px #4ECDC480" : "none",
              transition: "all 0.2s",
            }}>
              {label}{autoType === key ? " ✦" : ""}
            </button>
...
function RoutineEditor({ routines, setRoutines, storageKey }) {
  const [name, setName] = useState(""); const [minutes, setMinutes] = useState(5); const [icon, setIcon] = useState("⭐"); const [editing, setEditing] = useState(null);
  function addOrUpdate() {
    if (!name.trim()) return;
    let updated;
    if (editing) { updated = routines.map(r => r.id === editing ? { ...r, name: name.trim(), duration: Math.round(minutes * 60), icon } : r); setEditing(null); }
    else { updated = [...routines, { id: `r${Date.now()}`, name: name.trim(), duration: Math.round(minutes * 60), icon }]; }
  ...
  function move(idx, dir) { const a = [...routines], to = idx + dir; if (to < 0 || to >= a.length) return; [a[idx], a[to]] = [a[to], a[idx]]; setRoutines(a); save(storageKey, a); }
...
function LogPage({ weekday, weekend, logs, comments }) {
  ...
  const allRoutines = [...weekday, ...weekend];
  const uniqueRoutines = allRoutines.filter((r, i, arr) => arr.findIndex(x => x.name === r.name) === i);
  const stats = uniqueRoutines.map(r => {
    const completedDays = dates.filter(d => Object.values(logs[d]).some((e, _, arr) => logs[d][r.id]));
    const totalElapsed = dates.reduce((sum, d) => sum + (logs[d][r.id] ? (logs[d][r.id].elapsed || r.duration) : 0), 0);
    ...
    <div style={{ padding: "20px 16px 100px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
        {[{ val: dates.length, label: "記録した日数", color: "#4ECDC4" },
          { val: dates.reduce((sum, d) => sum + Object.values(logs[d]).reduce((s, e) => s + (e.elapsed || 0), 0), 0), label: "累計ルーティン時間", color: "#FF6B6B", isSec: true },
        ].map(s => (
      ...
      {dates.length === 0 ? (
        ...
        const type = getScheduleType(dateObj);
